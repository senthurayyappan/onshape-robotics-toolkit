...
def save_checkpoints_jsons(client: Client, checkpoint_path: str):
    if not os.path.exists(checkpoint_path):
        raise ValueError(f"Checkpoint path does not exist: {checkpoint_path}")

    checkpoint_df = pd.read_parquet(checkpoint_path, engine="pyarrow")
    checkpoint_df.dropna(inplace=True)

    json_dir = f"{os.path.splitext(checkpoint_path)[0]}_json"
    os.makedirs(json_dir, exist_ok=True)

    for _, row in tqdm(checkpoint_df.iterrows(), total=len(checkpoint_df)):
        try:
            for element_id in row["elementId"]:
                _, assembly_json = client.get_assembly(
                    did=row["documentId"],
...
if __name__ == "__main__":
    client = Client()
    save_checkpoints_jsons(client, "assemblies_checkpoint_0.parquet")
